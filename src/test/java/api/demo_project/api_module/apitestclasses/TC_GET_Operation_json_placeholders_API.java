package api.demo_project.api_module.apitestclasses;
...
import api.demo_project.api_module.apipageclasses.GET_Operation_UsersDataset;
import api.demo_project.api_module.apipageclasses.GET_operation_json_todosDataset;
import api.demo_project.api_module.apipageclasses.GET_operation_todosDataset;
public class TC_GET_Operation_json_placeholders_API extends BaseClass {
	...
	public TC_GET_Operation_json_placeholders_API() throws Exception {
		PropertyConfigurator.configure("log4j.properties");
		logger = Logger.getLogger(TC_GET_Operation_json_placeholders_API.class);
	...
	public void doRequest() throws Exception { 
		...
		String primaryInfo = GET_operation_todosDataset.primaryInfo;
		Constants.PRIMARY_INFO = primaryInfo;
		reports = ExtentConfigurations.getExtentInstance(reportsPath, projectPath ,GET_operation_todosDataset.projectName);
		int datasetLength = GET_operation_todosDataset.datasetLength; 
		for(int i = 1; i <= datasetLength; i++) {
		test = reports.startTest("TC_GET_Operation_json_placeholders_API - " + datasetNames[i-1]);

			boolean isExecutionFlag = true;			
		/*--------------------- GET --------------------- */

			if(isExecutionFlag) {
 			 	Object object = (Object) new GET_Operation_UsersDataset();
		...
		/*--------------------- GET --------------------- */
			...
			if(isExecutionFlag) {
 			 	Object object = (Object) new GET_operation_json_todosDataset();
				Class<?> c = object.getClass();
				String urlParams = (String) c.getField("urlParams" + i).get(object);
				if(urlParams == null) return;
				String headers = (String) c.getField("headers" + i).get(object);
		...
		/*--------------------- GET --------------------- */
			...
			if(isExecutionFlag) {
 			 	Object object = (Object) new GET_operation_todosDataset();
				Class<?> c = object.getClass();
				String urlParams = (String) c.getField("urlParams" + i).get(object);
				if(urlParams == null) return;
				String headers = (String) c.getField("headers" + i).get(object);
