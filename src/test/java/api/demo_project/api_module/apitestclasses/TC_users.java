...
public class TC_users extends BaseClass {
	...
	public TC_users() throws Exception {
		PropertyConfigurator.configure("log4j.properties");
		logger = Logger.getLogger(TC_users.class);
	...
	public void doRequest() throws Exception { 
		...
		String primaryInfo = GET_Operation_usersDataset.primaryInfo;
		Constants.PRIMARY_INFO = primaryInfo;
		reports = ExtentConfigurations.getExtentInstance(reportsPath, projectPath ,GET_Operation_usersDataset.projectName);
		int datasetLength = GET_Operation_usersDataset.datasetLength; 
		for(int i = 1; i <= datasetLength; i++) {
		test = reports.startTest("TC_users - " + datasetNames[i-1]);
